...
author: "Aoji Tang,Chenggze Xie"
date: "2024-11-09"
output: html_document
---

```{r setup, include=FALSE}
source("../scripts/prepare_lib.R")
```
...
## Results of Departmental and Semantic Analysis
```{r echo=FALSE}
source(paste0(mother_path, "/scripts/semantic_analysis"))
```

### Departmental Dominance


The analysis of departmental dominance involved calculating the proportion of emails sent and received by each user, categorized by department. The department with the highest percentage was identified as the dominant department for each user. The results are displayed in the following visualizations for sender and receiver distributions:


1. **Sender Distribution**: Shows the dominant departments for users based on email-sending behavior.
```{r}
persent_dom_plot_receiver
```
2. **Receiver Distribution**: Highlights the dominant departments based on email-receiving patterns.

```{r}
persent_dom_plot_sender
```
Then, we compare the above result and re-label everyone by the percentage that they are categorized into.
```{r}
head(final_data)
```

**Observations**:
- The **Operations** department is the most active in both sending and receiving emails, indicating its central role in communication networks.
- Departments like **Legal** and **Human Resources** exhibit limited communication activity, reflecting their specialized and targeted functions within the organization.

---
### Inter-Department Communication Patterns

To explore communication flows between departments, the frequency of interactions was quantified and visualized using a heatmap. Communication intensity was categorized into four bins: Zero, Low, Medium, and High.
```{r}
Inter_depart_commu_heapmap
```
As the more directly inspectation, here is the dataframe show the flow of emails among different department:
```{r}
head(directional_patterns)
```


**Findings**:
- **Central Communication Hubs**: The **Operations** department acts as a central node, maintaining extensive communication with almost all other departments.
- **Specialized Departments**: Departments like **Legal** and **Human Resources** are relatively isolated, likely due to their focused areas of responsibility.
- **Directional Patterns**: Communication between departments, such as **Trading** and **Operations**, exhibits noticeable asymmetry, suggesting unidirectional dependencies or task-specific interactions.

---

### Sentiment Analysis Between Departments

Sentiment analysis was conducted to evaluate the emotional tone of inter-departmental communications. Each tone was assigned a sentiment score:
- **Supportive**: \( +1.0 \) (Strongly Positive)
- **Deferential**: \( +0.5 \) (Mildly Positive)
- **Informative**: \( -0.5 \) (Mildly Negative)
- **Imperative**: \( -1.0 \) (Strongly Negative)



A heatmap was used to display the average sentiment scores across sender and receiver departments.
```{r}
sentiment_heatmap
```
**Insights**:
- **Positive Sentiments**: Communications initiated by the **Finance** department frequently display supportive tones, reflecting a constructive communication style.
- **Negative Sentiments**: Departments like **Legal** and **Trading** often receive negative sentiment tones, likely due to their roles in compliance or task management.
- **Neutral Communications**: Departments such as **Technology** and **Operations** demonstrate a relatively neutral tone, consistent with their functional and transactional nature.

---

### Tone Distribution Across Departments

The tone distribution analysis highlighted the communication styles prevalent within different departments. The proportion of each tone type (Supportive, Deferential, Informative, Imperative) was calculated and visualized for each sender department.
```{r}
tone_distribution_plot
```

*Observations**:
- **Supportive Tones**: Predominant in departments like **Engineering** and **Finance**, showcasing a culture of collaboration and teamwork.
- **Balanced Tone Usage**: Departments such as **Technology** and **Trading** display a more even distribution of tones, reflecting diverse and adaptable communication needs.
- **Imperative Tones**: The **Legal** department relies heavily on imperative tones, aligning with its role in issuing directives and handling compliance-related matters.
...
1. The **Operations** department is a pivotal hub, facilitating communication across the organization.
2. Positive sentiment dominates the organizational culture, with supportive tones frequently observed in departments like **Finance** and **Human Resources**.
3. Distinct tone usage patterns reflect the unique functions and responsibilities of each department, providing insights into organizational dynamics and communication strategies.
